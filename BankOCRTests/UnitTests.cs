...
namespace BankOCRTests
{
    ...
    public class UnitTests
    {
        ...
        public void ReadFile()
        {
            ...
            var accountDetails = processor.ParseFile(fileName).ToList();

            Assert.IsTrue(accountDetails.Count == 14);
            Assert.IsTrue(accountDetails[0].Length == 27 * 3);
            Assert.IsTrue(accountDetails[1] == " _  _  _  _  _  _  _  _  _ |_ |_ |_ |_ |_ |_ |_ |_ |_  _| _| _| _| _| _| _| _| _|");
        }
        ...
        public void WritevalidatedAccountInfo(string accountNo, string expected)
        {
            ...
            OcrProcessor processor = new OcrProcessor();
            var result = processor.ValidateAccounts(new List<AccountNumber>() { accountNumber })!;
            Assert.IsTrue(result[0].ToString() == expected);
        }
        ...
        public void ValidateAndRepairOcr(string ocrDigits, string result)
        {
            ...
            var accountStatus = processor.ValidateAccounts(new List<AccountNumber> { accountNumber });
            processor.RepairAccounts(accountStatus);
